...
d3.csv("data.csv").then(function (data) {
    // Count number of occurrences for each category for all states
    var totalCount = data.length;
    var BothCount = data.filter(function (d) {
        return d.HadDiabetes === "Yes" && d.HadHeartAttack === "Yes";
    }).length;

    var OnlyDiabetes = data.filter(function (d) {
        return d.HadDiabetes === "Yes" && d.HadHeartAttack === "No";
    }).length;

    var OnlyHeartAttack = data.filter(function (d) {
        return d.HadDiabetes === "No" && d.HadHeartAttack === "Yes";
    }).length;

    var NoneCount = data.filter(function (d) {
